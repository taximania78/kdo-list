...
// The shared "commune" list gets a group icon; every other list is a gift.
// Lists are runtime data, so no per-person slugs are hardcoded here.
const listIcons: Record<string, React.ReactNode> = {
  'commune': <Users className="w-10 h-10" />,
};

// Color palettes are assigned deterministically from the slug, so each list
// keeps a stable, distinct color without tying styling to any specific owner.
...
const slugColorIndex = (slug: string, count: number) => {
  let hash = 0;
